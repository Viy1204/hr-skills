...
- **`liepin` 无参**：打印帮助并退出（**没有交互式 REPL**，所有命令都是一次性执行）
- **一次性命令**：`liepin <subcommand> ...`，执行完即退出
- **全局选项**：`--json` 以结构化 JSON 输出（便于 AI Agent 解析）；`--help` / `-h` 打印帮助；`--version` / `-v` 打印版本
...
```bash
# usercId = search / recommend / talent 返回的 user_id
liepin greet <usercId> --ejobId <jobId>
```

`--ejobId` 强烈建议传，作用是关联到具体职位 + 触发平台对招呼语 / 权限的校验；不传也能跑（走默认路径），但**有些职位不传 ejobId 会失败**（platform 校验）。LPT 接口走 `com.liepin.im.b.chat.to-chat2`，**使用职位预设招呼语，不支持自定义消息文本**——如需个性化文案，先打招呼建立会话，再在 IM 里发。
...
下表参数名与代码 `src/toolset/*.ts` 中的 `*Options` 字段完全一致；位置参数用 `<>` 标记，命名参数用 `--` 前缀。

| 命令 | 参数 | 类型 | 默认 | 说明 |
|------|------|------|------|------|
| `search` | `query` | 位置 | — | 关键词（**必填**）|
| | `--city` | 命名 | — | 城市（如：北京、上海） |
| | `--experience` | 命名 | — | 工作经验（如：3-5年） |
| | `--salary` | 命名 | — | 薪资范围（如：20-30K） |
| | `--degree` | 命名 | — | 学历（如：本科） |
| | `--industry` | 命名 | — | 行业 |
| | `--page` | 命名 | 1 | 页码（0 基，内部会 -1）|
| | `--limit` | 命名 | 20 | 返回条数（最大 40）|
| `recommend` | `--jobTitle` | 命名 | — | 岗位关键字（模糊匹配）|
| | `--jobId` | 命名 | — | 直接指定 ejobId |
| | `--page` | 命名 | 1 | 页码 |
| | `--limit` | 命名 | 20 | 返回条数（最大 40）|
| `talent` | `query` | 位置 | — | 关键词（**必填**）|
| | `--city` | 命名 | — | 城市 |
| | `--experience` | 命名 | — | 工作经验 |
| | `--salary` | 命名 | — | 薪资范围 |
| | `--degree` | 命名 | — | 学历 |
| | `--page` | 命名 | 1 | 页码 |
| | `--limit` | 命名 | 20 | 返回条数 |
| `resume` | `talentId` | 位置 | — | 简历 ID（`search` / `recommend` / `talent` 返回的 `resume_id`，**必填**；代码字段名为 `talentId`）|
| `chatlist` | `--limit` | 命名 | 30 | 返回条数 |
| `chatmsg` | `oppositeImId` | 位置 | — | 对方 imId（`chatlist` 返回的 `im_id`，**必填**）|
| | `--limit` | 命名 | 50 | 返回消息数（最大 100）|
| `greet` | `usercId` | 位置 | — | 候选人 `user_id`（**必填**；**字段名是 `usercId` 不是 `userId`**）|
| | `--ejobId` | 命名 | `''` | 关联职位 ID（**强烈建议传**；不传也能跑，部分职位不传会失败）|
| `joblist` | `--status` | 命名 | `''` | 职位状态过滤（10 审核中 / 11 招聘中 / 12 已暂停 / 13 已关闭 / 20 审核未通过）|
| | `--page` | 命名 | 1 | 页码 |
| | `--limit` | 命名 | 20 | 返回条数（最大 40）|
| `login` | `--timeout` | 命名 | 120 | 登录等待超时（秒）|
| `skill` | `action` | 位置 | `install` | **目前仅支持 `install`**，传 `uninstall` 等其他值会报错 |
...
```bash
# 默认复制到 ~/.claude/skills/liepin-cli/
# 卸载：手动删除该目录
# 自定义安装路径：
export LIEPIN_SKILL_TARGET_DIR="$HOME/my-skills/liepin-cli"
